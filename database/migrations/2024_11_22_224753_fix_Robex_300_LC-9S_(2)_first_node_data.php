...
return new class extends Migration {
    ...
    public function up(): void
    {
        DB::transaction(function () {
            ...
            // Удаляем компоненты, связанные с обслуживанием и ремонтом
            DB::table('components')
            ...
            DB::table('components')
                ->where('name', 'Проверка сальниковых уплотнений узлов и агрегатов')
                ->where('amount', 1)
                ->where('node_id', $nodeId)
                ->delete();
            ...
            DB::table('components')
                ->where('name', 'Проверить состояния гусениц, опорных и поддерживающих катков')
                ->where('amount', 8)
                ->where('node_id', $nodeId)
                ->delete();
            ...
            DB::table('components')
                ->where('name', 'Проверка болтов ходового мотора, подтянуть, заменить')
                ->where('amount', 11)
                ->where('node_id', $nodeId)
                ->delete();
